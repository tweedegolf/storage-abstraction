...
const img = "tests/data/image1.jpg";
...
// call API method
const test1 = (path: string): void => {
  readImage1(path)
    .then((buffer) => {
      console.log("1", path, buffer.length);
    })
    .catch((e) => {
      console.log("1", path, e.message);
    });
};
...
const test1a = async (path: string): Promise<void> => {
  try {
...
const test1b = async (path: string): Promise<void> => {
...
// call API method
const test2 = async (path: string): Promise<void> => {
