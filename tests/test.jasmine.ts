...
const runSuite = (config: StorageConfig): Promise<void> =>
  new Promise(resolve => {
    console.log(config);
    const storage = new Storage(config);
    describe(`testing ${storage.introspect("type")} storage`, () => {
      ...
      afterAll(async () => {
        ...
        //     rimraf(
        //       path.join(dir, slugify(storage.introspect("bucketName") as string)),
        //       (e: Error) => {
        //         if (e) {
        //           reject();
        //           throw e;
    ...
    console.log(`suite ${storage.introspect("type")} completed`);
    resolve();
  });

const start = async (): Promise<void> => {
  for (let i = 0; i < configs.length; i++) {
    await runSuite(configs[i]);
  }
  console.log("done");
  process.exit(0);
};
