...
describe(`testing Amazon urls`, () => {
  ...
  //   expect((storage.config as ConfigAmazonS3).accessKeyId).toBe("key");
  //   expect((storage.config as ConfigAmazonS3).secretAccessKey).toBe(
  //     "secret/can/contain/slashes"
  //   );
  //   expect((storage.config as ConfigAmazonS3).region).toBe("");
  // });
  ...
  it("[1] parameter string", () => {
    ...
    expect((storage.config as ConfigAmazonS3).accessKeyId).toBe("key");
    expect((storage.config as ConfigAmazonS3).secretAccessKey).toBe("secret/can/contain/slashes");
    expect((storage.config as ConfigAmazonS3).region).toBe("eu-west-2");
    expect((storage.config as ConfigAmazonS3).options?.sslEnabled as unknown as string).toBe(
      "true"
    );
  });
  ...
  //   expect((storage.config as ConfigAmazonS3).region).toBe("");
  //   expect((storage.config as ConfigAmazonS3).bucketName).toBe("the-buck");
  // });
  ...
  it("[3] non-existent keys will not be filtered anymore, nor will invalid typed values (e.g. a numeric value for useDualStack)", () => {
    ...
    expect((storage.config as ConfigAmazonS3).endpoint).toBe(undefined);
    expect((storage.config as ConfigAmazonS3).endPoint).toBe(
  ...
  it("[4] object", () => {
    ...
    expect((storage.config as ConfigAmazonS3).secretAccessKey).toBe("secret/can/contain/slashes");
    expect((storage.config as ConfigAmazonS3).region).toBe("eu-west-2");
    expect((storage.config as ConfigAmazonS3).sslEnabled).toBe(true);
  });
  ...
  //   expect((storage.config as ConfigAmazonS3).region).not.toBe("eu-west-2");
  // });
  ...
  it("[5b] no bucket URL plus queryString", () => {
    ...
    expect(storage.getSelectedBucket()).toBe("");
    expect((storage.config as ConfigAmazonS3).region).toBe("eu-west-2");
    expect((storage.config as ConfigAmazonS3).sslEnabled as unknown as string).toBe("true");
  });
  ...
  it("[6] number and boolean in config object keep their original type", () => {
    ...
    expect((storage.config as ConfigAmazonS3).optionNumber).toBe(42);
    expect((storage.config as ConfigAmazonS3).optionBoolean).toBe(true);
  });
