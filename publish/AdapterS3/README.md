...
An adapter that provides an abstraction layer over the API of the Amazon S3 cloud storage service. It supports Amazon S3 and partly S3 compatible providers. The supported partly S3 compatible providers are: Cloudflare R2, Cubbit and the S3 API of Backblaze B2 and MinIO. Other S3 compatible providers may work as well.
...
```typescript
// only for Amazon S3
const s = new Storage({ type: StorageType.AWS });
// with a config url:
...
>[!NOTE]
>Typically you do not need specify a value for `endpoint` if you use this adapter to connect to Amazon S3.

### S3 Compatible Storage

Cloudflare R2, Backblaze B2, Cubbit and MinIO are supported S3 compatible services. You can use the `AdapterAmazonS3` but you have to add a value for `endpoint` in the config.
