...
import { BlobGenerateSasUrlOptions, BlobSASPermissions, BlobServiceClient, ContainerCreateOptions, StorageSharedKeyCredential, } from "@azure/storage-blob";
...
export class AdapterAzureStorageBlob extends AbstractAdapter {
    ...
    constructor(config: string | ConfigAzureStorageBlob) {
        ...
        this.sharedKeyCredential = new StorageSharedKeyCredential((this.config as ConfigAzureStorageBlob).storageAccount, (this.config as ConfigAzureStorageBlob).accessKey);
        this.storage = new BlobServiceClient(`https://${(this.config as ConfigAzureStorageBlob).storageAccount}.blob.core.windows.net`, this.sharedKeyCredential);
    }
    ...
    async getFileAsURL(fileName: string): Promise<string> {
        ...
        let options: BlobGenerateSasUrlOptions = {
    ...
    async fileExists(name: string): Promise<boolean> {
        ...
        const data = await this.storage.getContainerClient(this.bucketName).getBlobClient(name).exists();
        return data;
    }
